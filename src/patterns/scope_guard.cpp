...
class Storage {
    ...
    void undo() {
        i_ = i1_;
        std::cout << " ==> storage undo() operation" << std::endl;
    }
...
class Index {
    ...
    void undo() {
        i_ = i1_;
        std::cout << " ==> Index undo() operation" << std::endl;
    }
    int get() const { return i_; }
    private:
    int i_ = 0;
...
// define several types of scope guards:
...
{
    std::cout << "ScopeGuard example:" << std::endl;
    ...
    try {
        ...
        I.insert(-42, FAIL_THROW); //if insert to Memory fails - undo the previous storage operation through FailScopeGuard
    } catch (...) {
    }
